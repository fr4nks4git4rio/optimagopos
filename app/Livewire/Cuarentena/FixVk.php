...
class FixVk extends Modal
{
    ...
    {
        $registro = Cuarentena::find($this->registro);
        $this->ip = $registro->ip;
        $this->cliente_id = $registro->cliente_id;
        $this->sucursal_id = $registro->sucursal_id;
        $this->terminal_id = $registro->terminal_id;
        ...
        $decoded = json_decode($registro->data, true);
    ...
    public function getRegistroDbProperty()
    {
        return Cuarentena::findOrNew($this->registro);
    }
    ...
    public function guardar()
    {
        ...
        $this->dispatch('show-toast', __('site.quarantine.fix.ticket_save_successfully'), 'success');
    ...
    public function guardarYReprocesar()
    {
        ...
        try {
            ...
            $procesador = new ProcesarTicketVkCuarentena($registro);
            $res = $procesador->execute();

            if ($res) {
                $data = $registro;
                $registro->delete(); // o marcar como resuelto, según tu flujo
                ...
                $this->dispatch('show-toast', __('site.quarantine.fix.ticket_process_successfully'), 'success');
                $this->dispatch('$refresh');
                $this->dispatch('closeModal');
            } else {
                $this->dispatch('show-toast', __('site.quarantine.fix.ticket_process_error'), 'danger');
                $this->dispatch('$refresh');
            }
        } catch (Exception $e) {
    ...
    public function descartar()
    {
        $registro = Cuarentena::find($this->registro);
        $registro->delete();
        $this->dispatch('show-toast', __('site.quarantine.fix.register_delete_successfully'), 'success');
        $this->dispatch('$refresh');
        $this->dispatch('hide-sub-modal', 'modal-descartar-registro');
        $this->dispatch('closeModal');
    }

    public function showDescartarRegistroModal()
    {
        $this->dispatch('show-sub-modal', 'modal-descartar-registro');
    }
