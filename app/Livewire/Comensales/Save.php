...
class Save extends Modal
{
    ...
    public function updatedConstanciaFiscal($value)
    {
        if ($value) {
            if ($value->getMimeType() != 'application/pdf') {
                $this->dispatch('show-toast', __('site.diners.save.invalid_file_type'), 'danger');
                return;
            }
            ...
            try {
                ...
                $this->regimen_fiscal_id = optional(RegimenFiscal::findByDescription($datos_fiscales['regimen_fiscal']))->id;
                $this->dispatch('show-toast', __('site.diners.save.fiscal_data_loaded'), 'success');
            } catch (Exception $e) {
                Log::error("cargando los datos fiscales desde documento fiscal. Error: {$e->getMessage()}");
                $this->dispatch('show-toast', __('site.diners.save.invalid_fiscal_document'), 'danger');
            }
        }
    }
    ...
    public function save()
    {
        ...
        try {
            ...
            $this->dispatch('show-toast', __('site.diners.save.client_saved_successfully'), 'success');
            if ($this->scope) {
                if ($comensalDB->wasRecentlyCreated) {
                    $this->dispatch('cliente-created', $comensalDB->id)->to($this->scope);
                } else {
                    $this->dispatch('cliente-updated', $comensalDB->id)->to($this->scope);
                }
            } else {
            ...
            $this->dispatch('show-toast', __('site.diners.save.client_save_failed'), 'danger');
        }
    }
