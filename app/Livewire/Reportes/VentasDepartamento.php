...
class VentasDepartamento extends Component
{
    public $perPages;
    public $order;
    public $sort;
    ...
    public function mount()
    {
        ...
        $this->sort = $this->sort ?? __('site.reports.sales_by_department.department');
        ...
        $this->sorts = [__('site.reports.sales_by_department.branch'), __('site.reports.sales_by_department.department')];
    ...
    {
        if (user()->cannot('reportsSalesByDepartment-viewAny')) {
    ...
    public function render()
    {
        ...
        return view('livewire.reportes.ventas-departamento', [
    ...
    public function query()
    {
        ...
        switch ($this->sort) {
            case __('site.reports.sales_by_operator.branch'):
                ...
                break;
            case __('site.reports.sales_by_operator.departamento'):
                if ($this->order == 'asc')
    ...
    {
        if (user()->cannot('reportsSalesByDepartment-print')) {
            $this->dispatch('show-toast', __('site.common.client_no_permissions'), 'danger');
            return;
        }
        $name = __('site.reports.sales_by_department.title');
        if (File::exists(public_path("$name.pdf"))) {
            File::delete(public_path("$name.pdf"));
        }
        $view = 'reports.reportes.ventas-departamento.pdf';
        ...
        $this->dispatch('show-sub-modal', 'pdf-ventas-departamento');
    }

    public function exportarExcel()
    {
        if (user()->cannot('reportsSalesByDepartment-export')) {
            $this->dispatch('show-toast', __('site.common.client_no_permissions'), 'danger');
            return;
        }
        $name = __('site.reports.sales_by_department.title');
        ...
        return (new VentasDepartamentoExport($name, $this->sorts, $res['finalRecords'], $res['grandTotal'], $this->fechaInicio, $this->fechaFin, $sucursalesSeleccionadas))
            ->download($fileName);
    }
}
