...
class Index extends Component
{
    ...
    {
        if (in_array($field, ['search', 'perPage', 'sort', 'order', 'filter']))
            $this->resetPage();
    }

    public function render()
    {
        $records = $this->query()->groupBy('p.id')->get();

        $currentPage = $this->getPage();
        $total = $records->count();
        $currentItems = $records->forPage($currentPage, $this->perPage)->values();

        $paquetes = new LengthAwarePaginator($currentItems, $total, $this->perPage, $currentPage, ['path' => LengthAwarePaginator::resolveCurrentPath()]);
