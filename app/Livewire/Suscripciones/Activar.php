...
class Activar extends Modal
{
    ...
    public function activar()
    {
        ...
        try {
            ...
            $this->dispatch('show-toast', 'Suscripción activada.');
            if ($this->scope)
                $this->dispatch('suscripcion-activated', $this->suscripcion->id)->to($this->scope);
            $this->dispatch('$refresh');
            $this->dispatch('closeModal');
        } catch (\Exception $e) {
            DB::rollBack();
            Log::error("Error al intentar activar la suscripcion. Error: {$e->getMessage()}. " . json_encode($e->getTrace()));
            $this->dispatch('show-toast', 'Error al activar la suscripción.', 'danger');
            return;
        }
    }
}
