...
class AutoFacturacion extends Component
{
    ...
    public function init()
    {
        if ($this->comensal['direccion_fiscal']['estado_id']) {
            ...
            $this->dispatch("set-data-comensal-direccion_fiscal-estado_id", ['data' => [$estado->only('id', 'text')], 'term' => '', 'value' => $estado->id]);
        }
        if ($this->comensal['direccion_fiscal']['localidad_id']) {
            $localidad = Localidad::find($this->comensal['direccion_fiscal']['localidad_id']);
            $this->dispatch("set-data-comensal-direccion_fiscal-localidad_id", ['data' => [$localidad->only('id', 'text')], 'term' => '', 'value' => $localidad->id]);
        }
        if ($this->comensal['direccion_fiscal']['municipio_id']) {
            $municipio = Municipio::find($this->comensal['direccion_fiscal']['municipio_id']);
            $this->dispatch("set-data-comensal-direccion_fiscal-municipio_id", ['data' => [$municipio->only('id', 'text')], 'term' => '', 'value' => $municipio->id]);
        }
        if ($this->rfc)
        ...
        if ($this->facturar_por_forma_pago) {
            ...
            foreach ($formasPago as $fp) {
                ...
                $factura = Factura::create([
                    ...
                    'moneda' => $moneda,
                    'estado' => 'CAPTURADA',
                    'modo_prueba_cfdi' => modo_facturacion($propietario->id) != 1,
