...
class CabeceraFactura extends Component
{
    ...
    public function updated($field, $value)
    {
        if (preg_match('/^sucursales\.\d+\.formas_pago\.\d+\.forma_pago_id$/', $field)) {
            ...
            $this->dispatch('show-toast', 'Forma de pago actualizada', 'success');
        }
        $this->dispatch('reApplySelect2');
        $this->init();
    }
    ...
    public function init()
    {
        ...
        foreach ($this->sucursales as $index => $sucursal) {
            if ($sucursal['direccion']['estado_id']) {
                ...
                $this->dispatch("set-data-sucursales-$index-direccion-estado_id", ['data' => [$estado->only(['id', 'text'])], 'term' => '', 'value' => $estado->id]);
            }
            if ($sucursal['direccion']['localidad_id']) {
                $localidad = Localidad::find($sucursal['direccion']['localidad_id']);
                $this->dispatch("set-data-sucursales-$index-direccion-localidad_id", ['data' => [$localidad->only(['id', 'text'])], 'term' => '', 'value' => $localidad->id]);
            }
            if ($sucursal['direccion']['municipio_id']) {
                $municipio = Municipio::find($sucursal['direccion']['municipio_id']);
                $this->dispatch("set-data-sucursales-$index-direccion-municipio_id", ['data' => [$municipio->only(['id', 'text'])], 'term' => '', 'value' => $municipio->id]);
            }
        }
    }
    ...
    public function saveDatosGenerales($index)
    {
        ...
        $this->dispatch('show-toast', __('site.invoice_header.general_data_saved'), 'success');
    ...
    public function saveDireccion($index)
    {
        ...
        $this->dispatch('show-toast', __('site.invoice_header.address_saved'), 'success');
        //        $this->dispatch('$refresh');
    }
}
