...
class SaveComplemento extends Component
{
    ...
    public function loadInitialData()
    {
        if ($this->complemento->exists) {
            if ($this->cliente_id) {
                $this->dispatch('set-data-cliente_id', ['data' => [Cliente::find($this->cliente_id)->only('id', 'text')], 'term' => '', 'value' => $this->cliente_id]);
            }
            if ($this->serie_id) {
                $this->dispatch('set-data-serie_id', ['data' => [Serie::find($this->serie_id)->only('id', 'text')], 'term' => '', 'value' => $this->serie_id]);
            }
            if ($this->cfdi_id) {
                $this->dispatch('set-data-cfdi_id', ['data' => [Cfdi::find($this->cfdi_id)->only('id', 'text')], 'term' => '', 'value' => $this->cfdi_id]);
            }
            if ($this->forma_pago_id) {
                $this->dispatch('set-data-forma_pago_id', ['data' => [FormaPago::find($this->forma_pago_id)->only('id', 'text')], 'term' => '', 'value' => $this->forma_pago_id]);
            }
            if ($this->cliente_id) {
            ...
            // $this->dispatch('set-data-serie_id', ['data' => [Serie::find(4)->only('id', 'text')], 'term' => '', 'value' => 4]);
        }
    }
    ...
    public function comprobarSaldoFactura($index)
    {
        if ($this->saldoFactura($index) < 0) {
            $this->facturas[$index]['importe_pagado'] = 0;
            $this->dispatch('show-toast', 'El Saldo no puede ser menor que 0.', 'danger');
            ...
            $this->dispatch('show-toast', __('site.invoices.save_complement.several_partialities_not_allowed'), 'warning');
        }
    ...
    public function saveComplemento($timbrando = false)
    {
        ...
        $this->dispatch('show-toast', __('site.invoices.save_complement.complement_saved'));
        if (!$timbrando)
    ...
    public function eliminarFactura($id)
    {
        ...
        if (!$this->complemento->exists || $factura['can_be_removed']) {
            ...
            $this->dispatch('unselect-factura', $id);
        } else {
            $this->dispatch('show-toast', __('site.invoices.save_complement.cant_delete_related_invoice'), 'danger');
        }
    }
        ...
        if ($this->factura_conceptos[$index]['cotizacion'] != null) {
            foreach ($this->cotizaciones as $i => $element) {
                if ($element['id'] == $this->factura_conceptos[$index]['cotizacion']['id']) {
                    ...
                    $this->comentarios = Str::replace("Cotización " . $element['consecutivo'], '', $this->comentarios);
                    $this->dispatch('unselect-cotizacion', $element['id']);
                }
            }
        }
    ...
    {
        if (!$consecutivo) {
            $this->dispatch('nuevo-consecutivo-factura', $id, 'facturacion.prefacturas.save-complemento');
            return;
        }
