...
class CabeceraFactura extends Component
{
    ...
    public function init()
    {
        ...
        if ($this->direccion['estado_id']) {
            ...
            $this->dispatch("set-data-direccion-estado_id", ['data' => [$estado->only(['id', 'text'])], 'term' => '', 'value' => $estado->id]);
        }
        if ($this->direccion['localidad_id']) {
            $localidad = Localidad::find($this->direccion['localidad_id']);
            $this->dispatch("set-data-direccion-localidad_id", ['data' => [$localidad->only(['id', 'text'])], 'term' => '', 'value' => $localidad->id]);
        }
        if ($this->direccion['localidad_id']) {
            $municipio = Municipio::find($this->direccion['localidad_id']);
            $this->dispatch("set-data-direccion-municipio_id", ['data' => [$municipio->only(['id', 'text'])], 'term' => '', 'value' => $municipio->id]);
        }
    }
    ...
    public function saveDireccion()
    {
        ...
        if (!$owner) {
            $this->dispatch('show-toast', __('site.invoice_header.save_general_data_first'), 'danger');
            return;
        }
