...
class Save extends Component
{
    ...
    public function getFormWithErrorsProperty()
    {
        ...
        if ($withErrors)
            $this->dispatch('show-toast', __('site.invoices.save_invoice.errors_in_form'), 'danger');
        return $withErrors;
    }
    ...
    public function setSelect2Values()
    {
        if ($this->cliente_id) {
            ...
            $this->dispatch('set-data-cliente_id', [
    ...
    public function saveFactura($timbrando = false)
    {
        if ($this->con_facturas_relacionadas && !$this->tipo_relacion_factura_id) {
            $this->dispatch('show-toast', 'Debe seleccionar el Tipo de Relación.', 'warning');
            return;
        }
        ...
        $this->dispatch('show-toast', 'Pre-Factura guardada.', 'success');
        $this->dispatch('resetViewDataChange');
    ...
    {
        if ($index === null) {
            $this->dispatch('nuevo-concepto-factura', 'facturas-sistema.save')->to('facturas-sistema.save-factura-concepto');
        } else {
            $this->dispatch('editar-concepto-factura', $index, $this->factura_conceptos[$index], 'facturas-sistema.save')->to('facturas-sistema.save-factura-concepto');
        }
    }
        ...
        if ($this->factura_conceptos[$index]['suscripcion'] != null) {
            foreach ($this->suscripciones as $i => $element) {
                if ($element['id'] == $this->factura_conceptos[$index]['suscripcion']['id']) {
                    ...
                    $this->comentarios = Str::replace("Suscripción " . $element['paquete'], '', $this->comentarios);
                    $this->dispatch('unselect-suscripcion', $element['id']);
                }
            }
        }
        array_splice($this->factura_conceptos, $index, 1);
    ...
    {
        if (!$consecutivo) {
            $this->dispatch('nuevo-consecutivo-factura', $id, 'facturas-sistema.save');
            return;
        }
