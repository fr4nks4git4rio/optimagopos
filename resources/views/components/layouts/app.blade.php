...
window.addEventListener('resize', () => setModeScreen());" x-on:keydown.escape.window="closeMobileSidebar()">
    ...
    <script>
        ...
        document.addEventListener('show.bs.modal', (event) => {
            // Se ejecuta tras un leve timeout para asegurar que el DOM de Bootstrap se haya montado por completo
            setTimeout(() => {
                const activeModals = document.querySelectorAll('.modal.show');
                const backdrops = document.querySelectorAll('.modal-backdrop');

                activeModals.forEach((modal, index) => {
                    // Incremento de 20 puntos por cada modal abierto
                    const baseZIndex = 1050 + (index * 20);

                    // Asigna el z-index al modal actual
                    modal.style.setProperty('z-index', (baseZIndex + 10).toString(), 'important');

                    // Asigna el z-index al backdrop correspondiente
                    if (backdrops[index]) {
                        backdrops[index].style.setProperty('z-index', baseZIndex.toString(),
                            'important');
                    }
                });
            }, 50);
