...
@php
    ...
    if ($on_change) {
        $attributes['wire:change'] = $on_change;
    }
    if ($is_alpine) {
        $attributes->merge(['x-model' => $model]);
    }
@endphp

<div class="mb-1">
    <div @if (!$dynamic || $is_alpine) wire:ignore @endif
        @if ($is_alpine) x-data="{ {{ $model }}:'' }" @else x-data="{}" @endif
        x-init="() => {
            ...
            window.addEventListener('clear-data-{{ $id }}', (event) => {
