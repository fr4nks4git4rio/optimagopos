    ...
    <x-slot:content>
        <div class="row" wire:init="init()">
            <div x-data="{ avatar_uploaded: false }" class="col-12 col-md-3 text-center mb-2"
                ...
                @if ($this->has_avatar)
                    <button type="button" class="btn btn-secondary mt-2" wire:click="removePhoto()">Quitar Avatar
                    </button>
                @endif
            </div>
            <div class="col-12 col-md-9">
                <div class="row mb-2">
                    @if (user()->is_super_admin)
                        <div class="col-sm-4">
                            <x-select2-component-modals label="Rol" placeholder="Seleccione..." class="form-control"
                                :options="$roles" model="rol_id" />
                        </div>
                        <div class="col-sm-8">
                            @if ($rol_id == 1)
