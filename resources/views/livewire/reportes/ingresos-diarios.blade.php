@section('title', __('site.reports.daily_income.title'))
...
<div wire:init="init">
    ...
    <div class="row justify-content-between">
        <div class="row mb-1 col-md-10">
            <div class="col-sm-2">
                <x-input label="{{ __('site.reports.daily_income.start_date') }}" type="date" :lazy="true"
                    model="fechaInicio" />
            </div>
            <div class="col-sm-2">
                <x-input label="{{ __('site.reports.daily_income.end_date') }}" type="date" :lazy="true"
                    model="fechaFin" />
            </div>
            <div class="col-sm-8">
                <x-select2-multiple label="{{ __('site.reports.daily_income.branches') }}"
                    ...
                    <tr>
                        @foreach ($formasPago as $formaPago)
                            <th class="text-end" style="white-space: nowrap !important">
                                {{ __('site.reports.daily_income.amount') }}</th>
                            <th class="text-center" style="white-space: nowrap !important">
                                {{ __('site.reports.daily_income.quantity') }}</th>
                        @endforeach
                    </tr>
                </thead>
            @endif
            <tbody>
                @forelse($records as $sucursal_id => $sucursalData)
                    ...
                    <tr class="table-success fw-bold">
                        <td class="text-end" colspan="2">{{ __('site.reports.daily_income.total') }}
            ...
            @if (count($records) > 0)
                <tfoot>
                    <tr class="table-dark fw-bold">
                        <td colspan="2" class="text-end">{{ __('site.reports.daily_income.grand_total') }}</td>
                        @foreach ($formasPago as $i => $formaPago)
    ...
    <div class="modal fade" id="pdf-ingresos-diarios">
