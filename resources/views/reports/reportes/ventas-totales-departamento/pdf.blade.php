...
<body>
    ...
    <div class="filters-box">
        ...
        @if (!empty($sucursalesSeleccionadas))
            &nbsp;&nbsp;|&nbsp;&nbsp;
            <strong>{{ __('site.reports.total_sales_by_department.branches') }}:</strong>
        ...
        @if (count($records) > 0)
            <thead>
                <tr>
                    ...
                    @endforeach
                    <th colspan="2" style="text-align: center">{{ __('site.reports.total_sales_by_department.sales') }}</th>
                </tr>
                <tr class="sub-header">
                    <th style="text-align: right">{{ __('site.reports.total_sales_by_department.amount') }}</th>
                    <th style="text-align: center">{{ __('site.reports.total_sales_by_department.quantity') }}</th>
                </tr>
            </thead>
        @endif
        ...
        <tbody>
            @forelse($records as $sucursal_id => $sucursalData)
                ...
                <tr class="subtotal-row">
                    <td colspan="2" style="text-align: right">{{ __('site.reports.total_sales_by_department.total') }}
        ...
        @if (count($records) > 0)
            <tfoot>
                <tr class="grand-total-row">
                    <td colspan="2" style="text-align: right">{{ __('site.reports.total_sales_by_department.grand_total') }}</td>
